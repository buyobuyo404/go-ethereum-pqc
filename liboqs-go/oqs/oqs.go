...
func (sig *Signature) GenerateKeyPair() ([]byte, error) {
	...
	sig.publicKey = make([]byte, sig.algDetails.LengthPublicKey)
	sig.secretKey = make([]byte, sig.algDetails.LengthSecretKey)

	rv := C.OQS_SIG_keypair(sig.sig,
		(*C.uint8_t)(unsafe.Pointer(&sig.publicKey[0])),
	...
	//return sig.secretKey, nil
}

//func (sig *Signature) GenerateKeyPair2() ([]byte, []byte, error) {
//	// 没有sig
//	publicKey := make([]byte, sig.algDetails.LengthPublicKey)
//	secretKey := make([]byte, sig.algDetails.LengthSecretKey)
//
//	rv := C.OQS_SIG_keypair(sig.sig,
//		(*C.uint8_t)(unsafe.Pointer(&publicKey[0])),
//		(*C.uint8_t)(unsafe.Pointer(&secretKey[0])))
//	if rv != C.OQS_SUCCESS {
//		return nil, nil, errors.New("can not generate keypair")
//	}
//
//	return publicKey, secretKey, nil
//}
//
//func (sig *Signature) GenerateKeyPair3() ([]byte, error) {
//	publicKey := make([]byte, sig.algDetails.LengthPublicKey)
//	secretKey := make([]byte, sig.algDetails.LengthSecretKey)
//
//	rv := C.OQS_SIG_keypair(sig.sig,
...
// 对GenerateKeyPair的改写, 生成PQC秘钥对, 返回值的顺序为私钥, 公钥, 和err
func (sig *Signature) GenerateKeyPairFinal() ([]byte, []byte, error) {
	sig.publicKey = make([]byte, sig.algDetails.LengthPublicKey)
	sig.secretKey = make([]byte, sig.algDetails.LengthSecretKey)
...
func (sig *Signature) ExportPublicKey() []byte {
